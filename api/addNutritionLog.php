...
if(isset($_GET["user_id"])){
    ...
    if(!isset($_GET["date"])){
    ...
    } else if(!isset($_GET["recipe_id"]) && !isset($_GET["ingredient_id"])){
        $errors["recipe_ingredient_id_error"] = "Recipe or Ingredient ID is required";
        ...
        if(!intval($user_id)){
            ...
            if(!$db->getUserFromID($user_id))
                ...
                $recipe_id = (isset($_GET["recipe_id"])) ? htmlentities($_GET["recipe_id"]) : NULL;
                $ingredient_id = (isset($_GET["ingredient_id"])) ? htmlentities($_GET["ingredient_id"]) : NULL;
                $date = htmlentities($_GET["date"]);
                $time_of_day = (isset($_GET["time_of_day"])) ? htmlentities($_GET["time_of_day"]) : NULL;
                $ret = $db->addNutritionLogItem($user_id, $date, $time_of_day, $recipe_id, $ingredient_id);
